...
* `00_test_install`: small [notebook](00_test_install/test_install.ipynb) / [script](00_test_install/test_install.py) for testing that all packages were installed properly when [preparing for the workshop](#preparing-for-the-workshop)
* `01_demo`: [notebook](01_demo/demo.ipynb) / [script](01_demo/demo.py) that will be discussed during the demo part of the workshop
...
`00_test_install/test_install.py` or the `00_test_install/test_install.ipynb` notebook. Both the script and the notebook require you to set the path to the directory where the sample data (~3GB) should be stored. In this and all other scripts, parts of the script where your input is expected are marked with `FILL_ME`. Set the path and run the script / all cells of the notebook.
